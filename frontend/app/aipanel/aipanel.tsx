...
import { WaveAIChatHistoryEntry, WaveUIMessage } from "./aitypes";
...
const formatHistoryTime = (updatedTs: number): string => {
    if (!updatedTs) {
        return "";
    }
...
const AIHistoryModal = memo(() => {
    ...
    const loadHistory = useCallback(async () => {
        ...
        setError(null);
        try {
            const entries = await model.listChatHistory();
            setHistory(entries ?? []);
        } catch (loadError) {
            console.error("Failed to load Wave AI history:", loadError);
            setError(t("Failed to load chat history"));
        } finally {
            setIsLoading(false);
        }
    }, [model]);
    ...
    const handleDelete = async (entry: WaveAIChatHistoryEntry) => {
        const ok = window.confirm(t("Delete this chat?"));
        if (!ok) {
            return;
        }
        try {
            await model.deleteChatHistory(entry.chatid);
            await loadHistory();
        } catch (deleteError) {
            console.error("Failed to delete Wave AI chat:", deleteError);
            setError(t("Failed to delete chat"));
        }
    };

    const handleRestore = async (entry: WaveAIChatHistoryEntry) => {
        try {
            await model.restoreChat(entry.chatid);
        } catch (restoreError) {
            console.error("Failed to restore Wave AI chat:", restoreError);
            setError(t("Failed to restore chat"));
        }
    };

    return (
        <div className="absolute inset-0 z-30 bg-black/55 flex items-start justify-center p-3">
            <div className="w-full max-w-md max-h-[85%] mt-10 bg-zinc-900 border border-zinc-700 rounded-md shadow-xl flex flex-col overflow-hidden">
                <div className="px-3 py-2 border-b border-zinc-700 flex items-center justify-between">
                    <div className="text-sm font-semibold text-primary flex items-center gap-2">
                        <i className="fa fa-clock-rotate-left text-accent"></i>
                        {t("Chat History")}
                    </div>
                    <button
                        onClick={() => model.closeChatHistory()}
                ...
                <div className="flex-1 overflow-y-auto">
                    {isLoading ? (
                        ...
                        <div className="px-4 py-6 text-sm text-red-300 text-center">{error}</div>
                    ) : history.length === 0 ? (
                        <div className="px-4 py-6 text-sm text-muted text-center">{t("No chat history")}</div>
                    ) : (
                        <div className="py-1">
                            {history.map((entry) => (
                                <div
                                    key={entry.chatid}
                                    className={`w-full px-3 py-2 hover:bg-zinc-800 flex items-start gap-2 ${
                                        entry.chatid === currentChatId ? "bg-zinc-800/80" : ""
                                    }`}
                                >
                                    <i className="fa fa-message text-muted mt-1 flex-shrink-0"></i>
                                    <button
                                    ...
                                    >
                                        ...
                                        <span className="block text-[11px] text-muted truncate">
                                            {formatHistoryTime(entry.updatedts)}
                                            {entry.model ? ` · ${entry.model}` : ""}
                                            {entry.apitype ? ` · ${entry.apitype}` : ""}
                                            {` · ${entry.messagecount ?? 0} ${t("messages")}`}
                                        </span>
                                    </button>
                                    <button
                                        onClick={(e) => {
                                        ...
                                        className="text-gray-500 hover:text-red-300 p-1 cursor-pointer"
                                        title={t("Delete")}
                                    >
...
AIHistoryModal.displayName = "AIHistoryModal";
