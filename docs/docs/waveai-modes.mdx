...
:::tip
On a new empty chat, the model selector can switch directly to any valid configured custom mode. Compatibility restrictions only apply after a chat already has messages, because switching providers mid-conversation can break the current model context.
:::
