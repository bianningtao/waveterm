...
function isWordDocument(mimeType: string, fileName?: string): boolean {
    return mimeType == WordMimeType || fileName?.toLowerCase().endsWith(".docx");
}

function isExcelWorkbook(mimeType: string, fileName?: string): boolean {
    return mimeType == ExcelMimeType || fileName?.toLowerCase().endsWith(".xlsx");
}
...
function iconForFile(mimeType: string): string {
    ...
    if (mimeType == "application/pdf") {
        ...
        return "file-word";
    } else if (isExcelWorkbook(mimeType)) {
        return "file-excel";
    } else if (
        mimeType.startsWith("text/") ||
...
export class PreviewModel implements ViewModel {
    ...
    async getSpecializedView(getFn: Getter): Promise<{ specializedView?: string; errorStr?: string }> {
        ...
        if (isWordDocument(mimeType, fileName)) {
            return { specializedView: "word" };
        }
        if (isExcelWorkbook(mimeType, fileName)) {
            return { specializedView: "excel" };
        }
