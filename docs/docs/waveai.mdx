...
Use the model selector in the Wave AI panel to switch between Wave cloud modes, BYOK modes, and local/custom modes.

- Empty chats can switch directly to any valid configured mode.
- Chats that already contain messages use compatibility mode to prevent switching to a provider/model that cannot continue the current context.
- Start a new chat when you want to move an existing workflow to a different incompatible provider.
...
## Chat History and Local Sessions

Wave AI keeps chat history local. The history list loads only lightweight metadata such as title, timestamp, model, message count, and the local session file path.

Full chat content is loaded only when you restore a specific chat. Session snapshots are archived as JSONL files under the Wave data directory:

```text
sessions/YYYY/MM/DD/waveai-YYYY-MM-DDTHH-MM-SS-<chatid>.jsonl
```

New chats use `New Chat` as the default title. After the first user message, Wave derives a short title from that first prompt without making an extra AI request.
...
- Chat history metadata and JSONL session snapshots are stored on your local machine
